    ...
    @else
        ...
        <div class="row">
            <div class="col-12 text-right" style="text-align: right!important;">
                <?php
                if(in_array(session('user_title'), ['M/S', 'M/S.'])) {
                    // $request->session()->put('user_title', 'M/S');
                    $user_title = 'M/S';
                } elseif(in_array(session('user_title'),['Miss'])) {
                    // $request->session()->put('user_title', 'Ms.');
                    $user_title = 'Ms.';
                } else {
                ...
                ?>
                {{ucwords(strtolower($user_title) ?? '').' '.ucwords(strtolower(session('user_name') ?? ''))}}
                <br>
