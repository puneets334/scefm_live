            ...
            <div class="dashboard-section">
                <div class="row">
                    <div class="col-12 col-sm-12 col-md-12 col-lg-12">
                        <div class="dash-card">
                            <div class="title-sec">
                                <h5 class="unerline-title"> Partial Court Working Days (Supplementary)</h5>
                            ...
                            <hr>
                            @php
                            $iframe_src_route_uri = 'vacation_supp/advance/alllist?mainhead='.$mainhead;
                            if(uri_string() == 'vacation_supp/advance/declinelist'){
                                $iframe_src_route_uri = 'vacation_supp/advance/get_declinelist?mainhead='.$mainhead;
                            }
                            $current_tab = uri_string(); // e.g., 'vacation_supp/advance' or 'vacation_supp/advance/declinelist'
                            @endphp
                            <div class="dash-card dashboard-section tabs-section">
                                <div class="tabs-sec-inner">
                                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                                        <li class="nav-item" role="presentation">
                                            <a href="{{ base_url('vacation_supp/advance?mainhead=' . $mainhead) }}" class="{{ ($current_tab == 'vacation_supp/advance') ? 'nav-link active' : 'nav-link' }}"> Partial Court Working Days (All Matters) - For Consent </a>
                                        </li>
                                        <li class="nav-item" role="presentation">
                                            <a href="{{ base_url('vacation_supp/advance/declinelist?mainhead=' . $mainhead) }}" class="{{ ($current_tab == 'vacation_supp/advance/declinelist') ? 'nav-link active' : 'nav-link' }}"> Partial Court Working Days (Declined Matters) </a>
                                        </li>
                                    </ul>
                                    <?php
...
<script>
    document.getElementById('mainhead').addEventListener('change', function() {
        ...
        const baseUrl = "<?= base_url('vacation_supp/advance'); ?>";
