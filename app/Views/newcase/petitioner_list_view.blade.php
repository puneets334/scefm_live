                        ...
                        foreach ($petitioner_details as $exp) {
                            ...
                            <tr>
                                ...
                                <?php if ($petitioner_details[0]['party_type'] == 'I') { ?>
                                    <td data-key="Petitioner Name"><?php echo_data(strtoupper($exp['party_name'])) ?><br/><?php echo_data($relation)?> <br/><?php echo_data(strtoupper($exp['relative_name'])); ?></td>
                                    <td data-key="Age / D.O.B"><?php echo_data($exp['party_age'] . ' Yrs (' . $dob . ' )'); ?></td>
                                <?php } elseif ($petitioner_details[0]['party_type'] == 'D1' || $petitioner_details[0]['party_type'] == 'D2') { ?>
                                    <td data-key="Petitioner Name"> <?php echo_data($exp['org_post_not_in_list']=='t'?$exp['org_post_name'].', ':strtoupper($exp['authdesc'].', ')) . echo_data(strtoupper($exp['org_dept_not_in_list']=='t'?$exp['org_dept_name']:$exp['deptname']));?><br/>
                                        <span style="color: red;"><?php echo_data($exp['party_type']=='D1'?'State Department':'Central Department');?></span>
                                    </td>
                                <?php } elseif ($petitioner_details[0]['party_type'] == 'D3') { ?>
                                    <td data-key="Petitioner Name"><?php echo_data($exp['org_post_not_in_list']=='t'?$exp['org_post_name'].', ':strtoupper($exp['authdesc'].', ')) . echo_data(strtoupper($exp['org_dept_not_in_list']=='t'?$exp['org_dept_name']:$exp['deptname']));?>
                                        <br/><span style="color: red;"><?php echo_data('Other Department');?></span>
                                    </td>
                                <?php } ?>
                                <td data-key="Contact">
                                    <?php
                                ...
                                if(isset($hidepencilbtn)) {
                                    if($hidepencilbtn!='true') {
                                        ...
                                        <td data-key="Action" class="efiling_search">
                                            <a href="<?php echo base_url('newcase/petitioner/'); ?>">Edit</a>
                                            &nbsp;&nbsp;&nbsp;
