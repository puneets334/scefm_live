...
class Stageslist_model extends Model {
    ...
    public function get_efilied_nums_stage_wise_list($stage_ids, $created_by) {
        ...
        $builder->SELECT(array('en.efiling_for_type_id', 'en.efiling_for_id', 'en.ref_m_efiled_type_id', 'en.efiling_no', 'en.efiling_year', 'en.registration_id', 'en.allocated_on', 'et.efiling_type',
        ...
        $builder->JOIN('dynamic_users_table as users', 'en.created_by=users.id', 'LEFT');
        $builder->JOIN('m_tbl_departments dp', 'en.sub_created_by=dp.dep_user_id', 'LEFT');
        $builder->JOIN('m_tbl_clerks cl', 'en.sub_created_by=cl.clerk_id', 'LEFT');
    ...
    public function get_complaint_efiling_civil_extra_party($regid) {
    	$builder = $this->db->table('tbl_efiling_civil_extra_party as en');
    	$builder->SELECT('count(en.id)');
    	$builder->WHERE('ref_m_efiling_nums_registration_id', $regid);
    ...
    public function get_efilied_nums_submitted_list($stages, $created_by) {
        $builder = $this->db->table('tbl_efiling_nums as en');
        $builder->SELECT(array('en.efiling_for_type_id', 'en.efiling_no', 'en.ref_m_efiled_type_id', 'en.efiling_year', 'en.registration_id', 'et.efiling_type', 'cs.stage_id',
    ...
    public function get_efiled_list($stage_ids, $efiled_type, $created_by) {
        $builder = $this->db->table('tbl_efiling_nums as en');
        $builder->SELECT(array('en.efiling_for_type_id', 'en.efiling_for_id', 'en.ref_m_efiled_type_id', 'en.efiling_no', 'en.efiling_year', 'en.registration_id', 'et.efiling_type',
    ...
    public function get_efiling_num_basic_Details($regid) {
        $builder = $this->db->table('tbl_efiling_nums as en');
        $builder->SELECT(array('en.efiling_no', 'en.ref_m_efiled_type_id', 'en.efiling_year', 'en.registration_id', 'et.efiling_type', 'cs.stage_id',
    ...
    public function get_Misc_Docs_Details($regid) {
        $builder = $this->db->table('tbl_misc_doc_filing misc');
        $builder->SELECT('misc.*,
        ...
        $builder->JOIN('tbl_efiled_docs docs', 'misc.ref_m_efiling_nums_registration_id=docs.ref_m_efiling_nums_registration_id', 'left');
        $builder->JOIN('tbl_efiling_nums efnum', 'misc.ref_m_efiling_nums_registration_id=efnum.registration_id', 'left');
        $builder->WHERE('misc.ref_m_efiling_nums_registration_id', $regid);
        $builder->orderBy('docs.uploaded_on');
        $query = $builder->get();
        if ($query->getNumRows()) {
            $result = $query->getResult();
            return $result;
    ...
    public function get_Misc_Docs_count($regid) {
        ...
        $builder->SELECT('count(docs.doc_id)');
        $builder->WHERE('docs.ref_m_efiling_nums_registration_id', $regid);
    ...
    public function get_Petitioner_Preview_Details($regid) {
        $builder = $this->db->table('tbl_efiling_civil as ec');
        $builder->SELECT("ec.pet_name, ec.ref_m_efiling_nums_registration_id,ec.pet_sex,ec.pet_gender, ec.pet_father_flag, ec.pet_father_name, 
        ...
        $builder->JOIN('tbl_cis_masters_values as dn', 'dn.registration_id=ec.ref_m_efiling_nums_registration_id', 'LEFT');
        $builder->WHERE('ec.ref_m_efiling_nums_registration_id', $regid);
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResult();
        } else {
    ...
    public function get_Respondent_Preview_Details($regid) {
        $builder = $this->db->table('tbl_efiling_civil as ec');
        $builder->SELECT("ec.ref_m_efiling_nums_registration_id,ec.res_name, ec.res_sex,ec.res_gender, ec.res_father_flag, ec.res_father_name, 
    ...
    public function get_Extra_Info_Preview_Details($regid) {
        $builder = $this->db->table('tbl_efiling_civil as ec');
        $builder->SELECT("ec.ref_m_efiling_nums_registration_id,ec.pet_passportno, ec.petpanno,ec.pet_fax, ec.pet_phone, ec.pet_country, 
    ...
    public function get_Subordinate_court_Preview_Details($regid) {
        $builder = $this->db->table('tbl_efiling_civil as ec');
        $builder->SELECT("ec.ref_m_efiling_nums_registration_id,ec.lower_cino, ec.lower_judge_name,ec.lower_court, ec.filing_case, ec.lower_court_dec_dt, 
    ...
    public function get_Acts_Preview_Details($regid) {
        $builder = $this->db->table('tbl_efiling_civil as ec');
        $builder->SELECT("ec.ref_m_efiling_nums_registration_id, ec.under_sec1,ec.under_sec2, ec.under_sec3, ec.under_sec4,
    ...
    public function get_Extra_Party_Preview_Details($regid) {
        $builder = $this->db->table('tbl_efiling_civil_extra_party as eparty');
        $builder->SELECT("eparty.*");
        $builder->WHERE('eparty.ref_m_efiling_nums_registration_id', $regid);
        $builder->WHERE('eparty.parentid', NULL);
        $builder->WHERE('eparty.display', 'Y');
        $builder->orderBy("eparty.type", "asc");
        $builder->orderBy("cast(eparty.party_id as float)", "asc");        
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResult();
        } else {
    ...
    function get_efiling_num_details($registration_id) {
        ...
        $builder->SELECT("en.*, et.efiling_type, cs.stage_id, cs.activated_on,"
        ...
        $builder->JOIN('(select registration_id, max(initial_defects_id) defects_id from efil.tbl_initial_defects where is_defect_cured is true and is_approved is false group by registration_id) as defect', 'defect.registration_id=en.registration_id', 'LEFT');
    ...
    function makePayment($payment_detail) {
        ...
        if ($output) {
            if (ENABLE_JUDICIAL_STAMPS && $payment_detail['payment_method_code'] == PAYMENT_METHOD_CODE_STAMP) {
                ...
                } elseif (getSessionData('efiling_details')['ref_m_efiled_type_id'] == E_FILING_TYPE_DEFICIT_COURT_FEE) {
    ...
    function get_efiled_by_user($user_id) {
        $builder = $this->db->table('dynamic_users_table as users')
            ->SELECT('first_name, last_name, moblie_number')
            ->WHERE('id', $user_id);
        $query = $builder->get();
        if ($query->getNumRows()) {
            $result = $query->getResult();
    ...
    function finalsubmitdeficitcourtfee($registration_id) {
        ...
        if ($this->db->affectedRows() > 0) {
            ...
            if ($this->db->insertID()) {
                ...
                $builder2 = $this->db->table('efil.tbl_initial_defects')
                    ->SELECT('max(initial_defects_id) max_id')
                    ->WHERE('registration_id', $registration_id)
                    ->WHERE('is_defect_cured', FALSE)
                ...
                if ($initial_defects_max_id != '' && $initial_defects_max_id > 0) {
                    ...
                    $builder3 = $this->db->table('efil.tbl_initial_defects')
                        ->WHERE('registration_id', $registration_id)
                        ->WHERE('initial_defects_id', $initial_defects_max_id)
                        ->SET($update_defect_data)
                        ->UPDATE();
                    if ($this->db->affectedRows() > 0) {
    ...
    public function get_receipt_file($regid) {
        $builder = $this->db->table('tbl_court_fees cfa')
            ->SELECT('cfa.payment_receipts_file,nums.efiling_no')
            ->JOIN('tbl_efiling_nums as nums', 'cfa.registration_id = nums.registration_id')
            ->WHERE('cfa.registration_id', $regid);
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResult();
        } else {
    ...
    public function get_filing_details($regid) {
        $builder = $this->db->table('tbl_efiling_nums nums')
            ->SELECT('nums.efiling_no ,nums.efiling_year')
    ...
    public function getPaymentDefect($regid) {
        $builder = $this->db->table('tbl_payment_defects')
            ->SELECT('defect_remark,defect_date')
            ->WHERE('is_defect_cured', 'FALSE')
            ->WHERE('registration_id', $regid);
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
    ...
    public function get_total_payment_detail_pdf_view($regid) {
        $builder = $this->db->table('tbl_court_fee_payment')
            ...
            ->WHERE('registration_id', $regid)
            ->WHERE('is_active', TRUE)
            ->WHERE('is_payment_defective', FALSE)
            ->orderBy('id', 'DESC');
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResultArray();
    ...
    public function get_payment_details($regid) {
        $builder = $this->db->table('tbl_court_fee_payment')
            ->SELECT('*')
            ->WHERE('is_active', TRUE)
            ->WHERE('payment_verified_by', NULL)
            ->WHERE('payment_verified_date', NULL)
            ->WHERE('registration_id', $regid);
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResultArray();
    ...
    public function get_payment_preview_details($regid) {
        $builder = $this->db->table('efi.tbl_court_fee_payment')
            ->SELECT('*')
            ->WHERE('is_active', TRUE)
            ->WHERE('registration_id', $regid)
            ->orderBy('id', 'DESC');
    ...
    function add_payment_details($data, $file_temp_name) {
        ...
        if ($result) {
            $builder = $this->db->table('tbl_court_fee_payment');
            if ($builder->INSERT($merge_array_data)) {
                if ($_SESSION['efiling_details']['ref_m_efiled_type_id'] == E_FILING_TYPE_NEW_CASE) {
                    $this->Common_model->session_for_steps(NEW_CASE_COURT_FEE);
                } elseif ($_SESSION['efiling_details']['ref_m_efiled_type_id'] == E_FILING_TYPE_MISC_DOCS) {
                    $this->Common_model->session_for_steps(MISC_BREAD_COURT_FEE);
                ...
                $builder1 = $this->db->table('esign_logs')
                    ->WHERE('ref_registration_id', $registration_id)
                    ->SET($signed_status)
                    ->UPDATE();
    ...
    function delete_payment_details($id, $uploaded_by, $registration_id) {
        ...
        $this->db->transStart();
        $builder = $this->db->table('tbl_court_fee_payment')
            ->SELECT('tbl_court_fee_payment.*')
            ->WHERE('tbl_court_fee_payment.id', $id)
            ->WHERE('registration_id', $registration_id)
            ->WHERE($receipt_uploaded_by, $uploaded_by)
            ->WHERE('payment_verified_by', NULL)
            ->WHERE('payment_verified_date', NULL);
        $query = $builder->get();
        if ($query->getNumRows()) {
            $result = $query->getResult();
            $builder1 = $this->db->table('tbl_court_fee_payment')
                ->WHERE('id', $id)
                ->WHERE($receipt_uploaded_by, $uploaded_by)
                ->WHERE('registration_id', $registration_id)
                ->WHERE('payment_verified_by', NULL)
                ...
                ->DELETE();
            $signed_status = array('is_data_valid' => FALSE);
            $builder2 = $this->db->table('esign_logs')
                ->WHERE('ref_registration_id', $registration_id)
                ->SET($signed_status)
                ->UPDATE();
    ...
    function get_current_stage($registration_id) {
        $builder = $this->db->table('tbl_efiling_case_status')
            ->SELECT('registration_id, tbl_efiling_case_status.stage_id, m_tbl_dashboard_stages.user_stage_name, m_tbl_dashboard_stages.admin_stage_name')
            ->JOIN('m_tbl_dashboard_stages', 'm_tbl_dashboard_stages.stage_id = tbl_efiling_case_status.stage_id')
            ->WHERE('tbl_efiling_case_status.registration_id', $registration_id)
            ->WHERE('tbl_efiling_case_status.is_active', TRUE)
            ->orderBy('tbl_efiling_case_status.status_id', 'DESC')
            ->LIMIT(1);
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResultArray();
    ...
    function get_stages($registration_id) {
        $builder = $this->db->table('tbl_efiling_case_status')
            ->SELECT('*')
            ->JOIN('m_tbl_dashboard_stages as ds', 'tbl_efiling_case_status.stage_id=ds.stage_id', 'LEFT')
            ->WHERE('tbl_efiling_case_status.registration_id', $registration_id)
            ->orderBy('tbl_efiling_case_status.activated_on', 'ASC');
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResultArray();
        } else {
    ...
    public function get_Case_detail($registration_id) {
        $builder = $this->db->table('tbl_efiling_nums as en')
            ->SELECT(array('en.efiling_no', 'ec.cino', 'en.ref_m_efiled_type_id', 'en.efiling_year', 'en.registration_id', 'et.efiling_type', 'cs.stage_id', 'cs.activated_on', 'ec.pet_name', 'ms.case_type_name', 'res_name', 'ms.cause_title'))
            ->JOIN('tbl_efiling_case_status as cs', 'en.registration_id = cs.registration_id')
            ->JOIN('m_tbl_efiling_type as et', 'en.ref_m_efiled_type_id=et.id')
            ->JOIN('tbl_efiling_civil as ec', 'en.registration_id = ec.ref_m_efiling_nums_registration_id', 'left')
            ->JOIN('tbl_misc_doc_filing as ms', 'ms.ref_m_efiling_nums_registration_id=en.registration_id ', 'left')
    ...
    public function get_Misc_Docs_Details_history($regid) {
        $builder = $this->db->table('tbl_misc_doc_filing misc')
            ...
            ->JOIN('tbl_efiled_docs docs', 'misc.ref_m_efiling_nums_registration_id=docs.ref_m_efiling_nums_registration_id', 'left')
            ->JOIN('tbl_efiling_nums efnum', 'docs.ref_m_efiling_nums_registration_id=efnum.registration_id', 'left')
            ->WHERE('misc.ref_m_efiling_nums_registration_id', $regid)
            ->orderBy('docs.uploaded_on');
        $query = $builder->get();
        if ($query->getNumRows()) {
            $result = $query->getResult();
            return $result;
    ...
    public function get_court_fee_payment_history($regid) {
        $builder = $this->db->table('tbl_court_fee_payment')
            ->SELECT('*')
            ->WHERE('tbl_court_fee_payment.registration_id', $regid)
            ->WHERE('tbl_court_fee_payment.is_active', TRUE)
            ->orderBy('id', 'DESC');
        $query = $builder->get();
        if ($query->getNumRows()) {
    ...
    public function get_police_station_details($regid) {
        $builder = $this->db->table('tbl_efiling_civil as ec')
            ->SELECT("ec.ref_m_efiling_nums_registration_id, ec.police_private,ec.dt_chargesheet,ec.offense_date, ec.fir_year, ec.fir_no,
            ...
            ->JOIN('tbl_cis_masters_values as mv', 'mv.registration_id=ec.ref_m_efiling_nums_registration_id', 'LEFT')
            ->WHERE('ec.ref_m_efiling_nums_registration_id', $regid);
        $query = $builder->get();
        if ($query->getNumRows() >= 1) {
            return $query->getResult();
        } else {
    ...
    function lodges_case($registration_id, $lodges_case_reason) {
        ...
        if ($this->db->affectedRows() > 0) {
            ...
            if ($this->db->insertID()) {
                ...
                $builder2 = $this->db->table('tbl_efiling_case_status')
                    ->INSERT($insert_data);
                if ($this->db->insertID()) {
                    $this->db->transComplete();
                }
    ...
    function register_otp_generation_entry() {
        ...
        $builder = $this->db->table('esign_logs')
            ->INSERT($data);
    ...
    function save_otp_generation_entry() {
        ...
        $builder = $this->db->table('esign_logs')
            ->where('id', $_SESSION['insert_id'])
            ->set($_SESSION['otp_generation'])
            ->update();
        return true;
    }
    ...
    function get_pg_txn_log_details($order_ref_no) {
        $builder = $this->db->table('tbl_pg_transaction_log')
            ->SELECT('txn_log_id, order_ref_no, txn_amt, pg_txn_ref_no,        
    ...
    function assign_efiling_number($efiling_for_type_id, $efiling_for_id) {
        $query = "SELECT users.id,  users.ref_m_usertype_id, users.admin_for_type_id, users.admin_for_id, count(allocated_on) efiling_num_count
                FROM dynamic_users_table as users
    ...
    function reset_affirmation_model($docid, $reg_id, $efile_no, $update_data, $type) {
        $this->db->transStart();
        $builder = $this->db->table('esign_logs')
            ->WHERE('ref_registration_id', $reg_id)
            ->WHERE('ref_efiling_no', $efile_no)
            ->WHERE('type', $type)
            ->SET($update_data)
            ->UPDATE();
        if ($_SESSION['efiling_details']['ref_m_efiled_type_id'] == E_FILING_TYPE_NEW_CASE) {
    ...
    function efiling_no_allocated($data, $ref_m_efiled_type_id) {
        $this->db->transStart();
        $builder = $this->db->table('tbl_efiling_allocation')
            ->INSERT($data);
        if ($this->db->insertID()) {
            $builder = $this->db->table('tbl_efiling_nums')
                ->WHERE('registration_id', $data['registration_id'])
                ->WHERE('ref_m_efiled_type_id', $ref_m_efiled_type_id)
                ->SET(array('allocated_to' => $_SESSION['login']['id'], 'allocated_on' => $data['allocated_on']))
                ->UPDATE();
            $this->db->transComplete();
        }
        if ($this->db->transStatus() === FALSE) {
    ...
    function get_allocated_history($registration_id) {
        $builder = $this->db->table('tbl_efiling_allocation ea');
        $builder->SELECT("ea.*,concat(users.first_name,' ',users.last_name) as admin_name");
        $builder->JOIN('dynamic_users_table as users', 'users.id = ea.admin_id', 'LEFT');
        $builder->WHERE('registration_id', $registration_id);
    ...
    public function get_efilied_nums_cis_admin($stage_ids, $admin_for_type_id, $admin_for_id, $efiling_type) {
        $builder = $this->db->table('tbl_efiling_nums as en');
        $builder->SELECT('en.efiling_for_type_id, en.efiling_for_id, en.efiling_no, en.ref_m_efiled_type_id, en.efiling_year, en.registration_id, '
                . 'et.efiling_type, '
        ...
        $builder->WHERE('en.ref_m_efiled_type_id', $efiling_type);
        $builder->whereIn('cs.stage_id', $stage_ids);
        $builder->orderBy('en.registration_id');
        $query = $builder->get();
        if ($query->getNumRows() > 0) {
            return $query->getResultArray();
        } else {
    ...
    public function get_establishments_of_admin($admin_for_type_id, $admin_for_id) {
        if ($admin_for_type_id == E_FILING_FOR_ESTABLISHMENT) {
            $builder = $this->db->table('m_tbl_establishments as estab');
            $builder->SELECT('estab.id estab_id, ' . E_FILING_FOR_ESTABLISHMENT . ' establishment_type, estab.est_code estab_code, estab.state_code state_code');
            $builder->WHERE('estab.display', 'Y');
            $builder->WHERE('estab.id', $admin_for_id);
        } elseif ($admin_for_type_id == E_FILING_FOR_HIGHCOURT) {
            $builder = $this->db->table('m_tbl_high_courts as hc');
            $builder->SELECT('hc.id estab_id , ' . E_FILING_FOR_HIGHCOURT . ' establishment_type, hc.hc_code estab_code, hc.state_code state_code');
            $builder->WHERE('hc.is_active', 'Y');
            $builder->WHERE('hc.id', $admin_for_id);
        }
        $query = $builder->get();
        if ($query->getNumRows() > 0) {
            return $query->getResult();
    ...
    public function get_establishment_district($est_code) {
        $builder = $this->db->table('m_tbl_districts as dc');
        $builder->SELECT('dc.*');
        $builder->JOIN('m_tbl_establishments as est', 'dc.id =est.ref_m_tbl_districts_id');
        $builder->WHERE('est.est_code', $est_code);
        $query = $builder->get();
        if ($query->getNumRows() > 0) {
            return $query->getResult();
        } else {
    ...
    public function get_court_fee($shcilrefno) {
        $builder = $this->db->table('tbl_court_fee_payment');
        $builder->SELECT("*");
        $builder->WHERE('shcilpmtref', $shcilrefno);
        $query = $builder->get();
        if ($query->getNumRows() > 0) {
            return $query->getResult();
        } else {
    ...
    function get_department_name($reg_id, $dep_user_id) {
        ...
        $builder->SELECT("dp.*");
        $builder->JOIN('tbl_efiling_nums as en', "dp.dep_user_id=en.sub_created_by");
        $builder->WHERE('en.registration_id', $reg_id);
        $builder->WHERE('en.sub_created_by', $dep_user_id);
        $builder->WHERE('en.created_by', $_SESSION['login']['id']);
        $query = $builder->get();
        if ($query->getNumRows() > 0) {
            return $query->getResultArray();
        } else {
    ...
    function get_clerk_name($reg_id, $user_id) {
        ...
        $builder->JOIN('tbl_efiling_nums as en', "users.id=en.sub_created_by");
        $builder->WHERE('en.registration_id', $reg_id);
        $builder->WHERE('en.sub_created_by', $user_id);
        $builder->WHERE('en.created_by', $_SESSION['login']['id']);
        $query = $builder->get();
