...
class IAModel extends Model {
    ...
    function generate_efil_num_n_add_case_details($diary_details, $curr_dt_time, $appearing_for = "", $intervenorName = "") {
        $this->db->transStart();
        ...
        if ($generated_efil_num['efiling_num']) {
            ...
            if (isset($registration_id) && !empty($registration_id)) {
                $misc_docs_ia_details = array(
                    ...
                    'ref_m_efiled_type_id' => E_FILING_TYPE_IA,
                ...
                if ($appearing_for == "I") {
                    $misc_docs_ia_details = array_merge($misc_docs_ia_details, array('intervenor_name' => $intervenorName, 'p_r_type' => $appearing_for));
                }
    ...
    function gen_efiling_number() {
        ...
        $builder->SELECT('ia_efiling_no,ia_efiling_year');
        ...
        $p_efiling_num = $row[0]['ia_efiling_no'];
        $year = $row[0]['ia_efiling_year'];
        if ($year < date('Y')) {
            $newYear = date('Y');
            $update_data = array(
                'ia_efiling_no' => 1,
                'ia_efiling_year' => $newYear,
                'ia_updated_by' => getSessionData('login')['id'],
                'ia_updated_on' => date('Y-m-d H:i:s'),
                'ia_update_ip' => getClientIP()
            );
            $builder = $this->db->table('efil.m_tbl_efiling_no');
            $builder->WHERE('ia_efiling_no', $p_efiling_num);
            $builder->WHERE('ia_efiling_year', $year);
            $builder->WHERE('entry_for_type', getSessionData('estab_details')['efiling_for_type_id']);
            $builder->WHERE('ref_m_establishment_id', getSessionData('estab_details')['efiling_for_id']);
    ...
    function add_misc_docs_ia_details($registration_id, $case_details, $breadcrumb_step, $appearing_for = "") {
        ...
        if ($this->db->insertID()) {
            ...
            if ($update_status) {
                ...
                if ($appearing_for == 'I') {
                    $update_status = $this->update_breadcrumbs($registration_id, IA_BREAD_ON_BEHALF_OF);
                    return $update_status;
                }
                if ($appearing_for == 'E') {
                    $exists_appearing = $this->check_existence_of_appearing_for($case_details['diary_no'], $case_details['diary_year']);
                    if ($exists_appearing) {
                        $update_status = $this->update_breadcrumbs($registration_id, IA_BREAD_APPEARING_FOR);
                        return $update_status;
                    } else {
                        return $update_status;
                    }
