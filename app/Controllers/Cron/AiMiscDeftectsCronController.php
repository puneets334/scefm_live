...
class AiMiscDeftectsCronController extends BaseController {
    ...
    public function index()
    {
        ...
        $cronDetailsId=$this->AiMiscDeftectsCron_model->insertInDBwithInsertedId('efil.tbl_cron_details',$cronDetails);
            ...
            $loop_for_stage_flag = [I_B_Approval_Pending_Admin_Stage,I_B_Defected_Stage,IA_E_Filed,Document_E_Filed];//defect notified
            $case_type = [E_FILING_TYPE_IA]; // 4 => IA
                $scrutiny_result = $this->AiMiscDeftectsCron_model->get_ai_misc_efiled_nums_stage_wise_list_admin_cron($loop_for_stage_flag,$case_type);
                if ($scrutiny_result) {
                    $case_chunks = array_chunk($scrutiny_result, 10);
                    foreach ($case_chunks as $index => $chunk) {
                        echo "<br/> Chunk No. " . ($index + 1) . " and Chunk size: " . sizeof($chunk);
                        $data = $this->efiling_webservices->get_ia_misc_efiling_scrutiny_cron_SCIS($chunk);
                        if ($data) {
                            $curr_dt = date('Y-m-d H:i:s');
                            foreach ($data->consumed_data as $response) {
                                if ($response->status == 'A' && $response->objection_flag == 'Y') {
                                    $objections_status = [];
                                    $next_stage = 0;
                                    ...
                                    $stage_update_timestamp = $this->Get_AiMiscDefects_CIS_Status_model->get_ai_misc_stage_update_timestamp($response->registration_id, $loop_for_stage_flag);
                                    ...
                                    if (isset($response->objection_flag) && $response->objection_flag == 'Y') {
                                        ...
                                        $objections_status = $this->get_icmis_ai_misc_objections_status($response->registration_id, $cis_objections, $curr_dt);
                                        ...
                                        $next_stage = Document_E_Filed; // 14=> Misc. Document E-Filed or IA both are same stage
                                    }
                                    ...
                                    $update_status = $this->AiMiscDeftectsCron_model->update_ai_misc_icmis_case_status($response->registration_id, $next_stage, $curr_dt, $case_details, $objections_status[1], $objections_status[2]);
                                    if ($update_status) {
                                        if ($next_stage) {
                    ...
                    echo "No Records Found for IA E-filed<br/>";
                }
    ...
    {
        $cronDetails = [
            "cron_type" => 'ia_miscDocs',
            'started_at' => date('Y-m-d H:i:s')
        ];

        $cronDetailsId = $this->AiMiscDeftectsCron_model->insertInDBwithInsertedId('efil.tbl_cron_details', $cronDetails);
            $loop_for_stage_flag = [I_B_Approval_Pending_Admin_Stage,I_B_Defected_Stage,IA_E_Filed,Document_E_Filed];//defect notified
            $case_type = [E_FILING_TYPE_MISC_DOCS]; // 2 => misc_document
            $scrutiny_result = $this->AiMiscDeftectsCron_model->get_ai_misc_efiled_nums_stage_wise_list_admin_cron($loop_for_stage_flag, $case_type);
                if ($scrutiny_result) {
                    $case_chunks = array_chunk($scrutiny_result, 10);
                    foreach ($case_chunks as $index => $chunk) {
                            ...
                            foreach ($data->consumed_data as $response) {
                                ...
                                if ($response->status == 'A' && $response->objection_flag == 'Y') {
                                    ...
                                    if (isset($response->objection_flag) && $response->objection_flag == 'Y') {
                                        ...
                                        $next_stage = Document_E_Filed; //$next_stage = I_B_Approval_Pending_Admin_Stage;
                                    }
                    ...
                    echo "No Records Found for Misc. Document E-Filed<br/>";
                }
