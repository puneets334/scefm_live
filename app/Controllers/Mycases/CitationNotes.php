...
namespace App\Controllers\Mycases;

use App\Controllers\BaseController;
use App\Models\MyCases\ListsModel;
use App\Models\Common\CommonModel;
use App\Models\MyCases\CitationNotesModel;

class CitationNotes extends BaseController
{

    protected $Lists_model;
    protected $CommonModel;
    protected $Citation_notes_model;
    protected $session;
    protected $db;
    protected $slice;
    ...
    public function add_case_contact()
    {
        ...
        $dno = $request->getPost('diary_no');
        $name = strtoupper($request->getPost('name'));
        $email = $request->getPost('email_id');
        $mobile = $request->getPost('mobile_no');
        $o_contact = $request->getPost('o_contact');
        $contact_type = strtoupper($request->getPost('contact_type'));
        ...
        $validate_contact_type = validate_names($contact_type, TRUE, 3, 99, 'Contact Type');
        if ($validate_contact_type['response'] == FALSE) {
            echo '1@@@' . htmlentities($validate_contact_type['msg']['field_name'], ENT_QUOTES);
            exit(0);
        }
        $contact_id = url_decryption($request->getPost('contact_id'));
        $data = [
        ...
        if (empty($contact_id)) {
            ...
            $data['created_by'] = $session->get('login.id');
            ...
            if ($result) {
                ...
                echo '1@@@' . htmlentities('Some Error Occurred!', ENT_QUOTES);
            }
        } else {
            ...
            if ($result) {
                ...
                echo htmlentities('Some Error Occurred!', ENT_QUOTES);
            }
        }
    }
    ...
    {
        $session = session();
        if ($session->get('login.ref_m_usertype_id') != USER_ADVOCATE && $session->get('login.ref_m_usertype_id') != USER_ADVOCATE_CIS) {
            return redirect()->to(base_url('/'));
            exit(0);
        }
        $request = \Config\Services::request();
        $type = $request->getPost("type");
        $dno = $request->getPost("cnr_number");
        if (empty($dno)) {
            echo '1@@@Invalid Diary Number';
            exit(0);
        }
        $header = '<i class="fa fa-users"></i> Case Contacts';
        $result = $this->Citation_notes_model->get_case_contact_data($session->get('login.id'), $dno);
        if (!empty($result)) {
            echo '2@@@';
            $i = 1;
            ?>
                ...
                <tbody>
                    ...
                    foreach ($result as $dataRes) {
                        ...
                        <tr>
                            ...
                            <td width="12%">
                                <a href="javascript:void(0)" onclick="get_contacts('<?php echo htmlentities($dataRes['id'], ENT_QUOTES); ?>')" title="View "><i class="fa fa-pencil"></i></a>
                                <?php echo '<a href="javascript:void(0);" onclick ="delete_contacts(\'' . $dataRes['id'] . '\',\'' . htmlentities($dataRes['diary_no'], ENT_QUOTES) . '\',\'' . ($type) . '\');" ><button type="button" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i> </button> </a>'; ?>
                            </td>
                        </tr>
                    <?php } ?>
                </tbody>
    ...
    public function case_contact()
    {
        ...
        { 
            echo '2@@@<div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" style="margin-left: 38%;"><i class="fa fa-users"></i> Edit Case Contacts </h4>
            ...
            echo '<div class="modal-body">
                <div class="row col-12" id="contact_div_hide">
                    <div class="col-md-12 col-sm-12 col-xs-12">
                        ...
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <input type="text" size="50" id="p_name" name="name" value="' . htmlentities($results[0]['p_name'], ENT_QUOTES) . '" class="form-control cus-form-ctrl" maxlength="250" placeholder="Name">
                            </div>
                        </div>
                        <div class="clearfix"></div><br>
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <input type="text" size="50" id="p_email_id" name="email_id" value="' . htmlentities($results[0]['p_email'], ENT_QUOTES) . '" class="form-control cus-form-ctrl" maxlength="250" placeholder="Email Id">
                            </div>
                        </div>
                        <div class="clearfix"></div><br>
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <input type="text" size="50" id="p_mobile_no" name="mobile_no" value="' . htmlentities($results[0]['p_mobile'], ENT_QUOTES) . '" class="form-control cus-form-ctrl" maxlength="10" placeholder="Mobile">
                            </div>
                        </div>
                        <div class="clearfix"></div><br>
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <input type="text" size="50" id="p_o_contact" name="o_contact" value="' . htmlentities($results[0]['p_other_contact'], ENT_QUOTES) . '" class="form-control cus-form-ctrl" maxlength="55"  placeholder="Other contact">
                            </div>
                        </div>
                        <div class="clearfix"></div><br>
                        <div class="col-md-12 col-sm-12 col-xs-12">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                ...
                                <input type="text" size="50" id="p_contact_type" name="contact_type" value="' . htmlentities($results[0]['contact_type'], ENT_QUOTES) . '" class="form-control cus-form-ctrl" maxlength="100" placeholder="Petitioner, Respondent, Witness, Other">
                            </div>
                        </div>
                        <div class="clearfix"></div><br>
                ...
                $("#edit_case_contact_details").on("submit", function(e) {
                    ...
                    if ($("#edit_case_contact_details").valid()) {
                        ...
                        $.ajax({
                            type: "POST",
                            data: form_data,
                            url: "' . base_url('mycases/add_case_contact') . '",
    ...
    /* public function add_citation() {
        $citation_description = script_remove($this->request->getPost("citation_description"));
        $remark_length = strip_tags($citation_description);
        $current_url = script_remove($this->request->getPost("current_url"));
        ...
        $dno = $this->request->getPost("citation_cnr_no");
        $description = array(
    ...
    {
        // $citation_description = $this->request->getPost("temp");
        //  $remark_length = strip_tags($citation_description);
        //   $current_url = script_remove($this->request->getPost("current_url"));
        /*  if (strlen($remark_length) > DISAPPROVE_REMARK_LENGTH) {
            $_SESSION['MSG'] = message_show("fail", 'Description should be max ' . DISAPPROVE_REMARK_LENGTH . ' characters!');
    ...
    public function add_notes()
    {
        ...
        $notes_description = script_remove($this->request->getPost("notes_description"));
        ...
        $dno = $this->request->getPost("notes_cnr_no");
        if (empty($dno)) {
    ...
    public function get_citation_and_notes_list()
    {
        ...
        $after_delete = $this->request->getPost("after_delete");
        if (!empty($after_delete)) {
            $type = url_decryption($this->request->getPost("type"));
            // $type=$this->request->getPost("type");
            $dno = url_decryption($this->request->getPost("cnr_number"));
        } else {
            $type = $this->request->getPost("type");
            $dno = $this->request->getPost("cnr_number");
        }
        if (empty($dno)) {
            echo '1@@@Invalid Diary Number';
            exit(0);
    ...
    public function get_recipients_mail()
    {
        ...
        $re_emails = '<div class="uk-panel-scrollable" uk-float-right><ul style="padding-left: 0 !important;">
            ...
            <a data-toggle="dropdown" class="dropdown-toggle">Select Contacts <b class="caret" style="float:right;"></b></a>
            <ul class="dropdown-menu col-xs-11">';
        $i = 1;
        $re_emails .= '<table border="1">';
        if (isset($get_contact) && !empty($get_contact)) {
            foreach ($get_contact as $contact) {
                if ($_POST['type'] == 'mail' && !empty($contact['p_email'])) {
                    $re_emails .= '<tr><td><div align="left" class="checkbox" style="margin-left: 45px;">
                        <input type="checkbox" value="' . $contact['p_email'] . '" name="emailMulti" id="email_' . $i . '" onclick="updateTextArea(' . $i . ', $(this))">' . $contact['p_name'] . '</div></td></tr>';
                }
                if ($_POST['type'] == 'sms' && !empty($contact['p_mobile'])) {
                    $re_emails .= '<tr><td><div align="left" class="checkbox" style="margin-left: 45px;">
                        <input type="checkbox" value="' . $contact['p_mobile'] . '" name="mobile[]" id="mob_' . $i . '" onclick="updatemobArea(' . $i . ', $(this))">' . $contact['p_name'] . '</div></td></tr>';
                }
                $i++;
            }
        }
        $re_emails .= '</table>';
        $re_emails .= '</ul></li></ul></div>';
        echo !empty($re_emails) ? $re_emails . "@@@" . str_replace(",,", ",", $re_emails) : '';
        echo "<script type='text/javascript'>
            function updateTextArea(id, email_checked) {
                ...
                var emailval = $('#emailids').html();
                $('#recipient_email').val(emailval);
            }    
    ...
    public function send_sms()    //function added by Preeti Agrawal on 2 jan 2021
    {
        ...
        if (isset($mobile_no) && !empty($mobile_no)) {
            ...
            for ($i = 0; $i < count($recp_mob_ids); $i++) {
                $strlen = strlen($recp_mob_ids[$i]);
                $isNumeric = is_numeric($recp_mob_ids[$i]);
                $onlyNumeric = preg_match('/^\d+$/', $recp_mob_ids[$i]) ? true : false;
                // $validate_nums = validate_number($recp_mob_ids[$i], TRUE, 10, 10, 'Recipient Mobile No.');
                // pr($validate_nums);
                if ($strlen != 10 || $isNumeric == FALSE || $onlyNumeric == false) {
                    // echo '1@@@' . htmlentities($validate_nums['msg']['field_name'], ENT_QUOTES);
                    echo '1@@@' . htmlentities('The Recipient Mobile No. field is required.', ENT_QUOTES);
                    exit(0);
                }
            }
        }
